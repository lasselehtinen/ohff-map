...
class UpdateActivations extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:update-activations {--all}';
    ...
    public function handle()
    {
        ...
        $references = ($this->option('all')) ? Reference::all() : Reference::where('latest_activation_date', '>=', now()->subDays(60)->startOfDay()->toDateTimeString())->get();
        ...
        $references->each(function ($reference, $key) use ($bar) {
            ...
            // Add operator activations
            $activations->each(function ($activation, $key) use ($reference) {
                // Create / fetch operator exists
                $operator = Operator::firstOrCreate(['callsign' => $activation['callsign']]);

                // Prevent duplicates
                $activationsOnReference = $operator->activations()->get()->filter(function ($activationReference, $key) use ($reference, $activation) {
